...
سرویس شده باشند.
یک \lr{VNF} حداکثر یک بار سرویس داده شود.
...
\section{اهمیت مساله}
اولین سوالی که از مساله‌ی حاضر مطرح است چرایی جایگذاری نکردن منابع مدیریتی به عنوان بخشی از زنجیره‌ها است.
از آنجایی که برای منابع مدیریتی هزینه‌ای مدنظر است نمی‌توان
آن‌ها را به عنوان بخشی از زنجیره‌ها جایگذاری کرد چرا که نیاز است اشتراک بین آن‌ها در جهت کاهش هزینه‌ها صورت بگیرد.

دومین سوال حل توامان است که آیا جایگذاری توامان منابع مدیریتی و زنجیره‌ها نسبت به حالتی که ابتدا زنجیره‌ها جایگذاری شده
در نهایت منابع مدیریتی جایگذاری شوند بهبودی دارد یا خیر.

با حل توامان می‌توان محدودیت‌های مدیریتی مشخصی تعریف نمود که در صورتی که مساله به صورت غیر توامان
(در دو مرحله جایگذاری زنجیره‌ها و منابع مدیریتی) حل شود توان اعمال آن‌ها وجود ندارد
از جمله‌ی این محدودیت‌ها می‌توان به محدودیت قرارگیری سرورهای \lr{VNFM} اشاره کرد.
به طور مثال در نمودار \ref{fig.20} می‌توان افزایش چشم‌گیر سود در راه حل بهینه توامان را با در نظر گرفتن این محدودیت‌ها دید.
بنابراین در صورتی که چنین محدودیت‌هایی وجود داشته باشد استفاده از راه‌حل غیرتوامان منطقی به نظر نمی‌رسد.

از آنجایی که راه حل غیر توامان در هنگام جایگذاری زنجیره‌ها توانایی در نظر گرفتن این محدودیت‌های مدیریتی را ندارد
در زمان جایگذاری منابع مدیریتی برای جلوگیری از ناممکن نشدن مساله
مجبور به در نظر نگرفتن تعدادی از زنجیره‌های انتخاب شده در مرحله قبل می‌شود
که به این ترتیب سود کاهش پیدا می‌کند.
برای آزمودن این فرضیه فرمول‌بندی مساله را به صورت غیرتوامان بازنویسی کرده و به آن امکان می‌دهیم تا در صورت نیاز زنجیره‌هایی که پیشتر انتخاب شده‌اند را در نظر نگیرد.

در صورتی که این امکان در نظر گرفته نشود مساله برای یک توپولوژی \lr{FatTree}
با مقدار \lr{k} برابر با ۶ و ۱۰۰ زنجیره غیرممکن است.

نمودار \ref{fig.20} از توپولوژی \lr{FatTree} با مقدار \lr{k} برابر با ۶ استفاده می‌کند.
در این توپولوژی برای قرارگیری \lr{VNFM}ها محدودیت در نظر گرفته شده است
و نمی‌توان‌ زنجیره‌ها را به وسیله‌ی \lr{VNFM}ای که در غلاف غیرمجاور وجود دارد مدیریت نمود.

\begin{figure}[!h]
\center\includegraphics[scale=0.5]{images/joint-fattree}
\caption{مقایسه راه‌حل توامان در شبکه \lr{FatTree}}
\label{fig.20}
\end{figure}

مساله‌ی توامان حتی بدون در نظر گرفتن محدودیت‌های مدیریتی و تنها با در نظر گرفتن نیازمندی منابع مدیریتی به منابع پردازش و حافظه باز هم می‌تواند سود بیشتری بدست آورد.
اگر فرض کنید زیرساخت گنجایش منابع مدیریتی و زنجیره‌ها به صورت همزمان را دارد، نمودار زیر سود حاصل از پذیرش زنجیره‌ها را نمایش می‌دهد.

این منابع مدیریتی به صورت متمرکز در نظر گرفته شده است و به فرمول‌بندی اجازه داده نمی‌شود که زنجیره‌ی پذیرفته شده‌ای را در نظر نگیرد.

در نمودار \ref{fig.21} از توپولوژی \lr{USnet} استفاده شده است و منابع پردازشی و حافظه‌ای به گونه‌ای در نظر گرفته شده‌اند
که از میزان مورد نیاز برای پذیرش زنجیره‌ها بسیار بیشتر نباشند.
بخشی از منابع در اولین گام در اختیار الگوریتم قرار نمی‌گیرد تا از آن در گام دوم برای نگاشت زنجیره‌ها استفاده شود.
این امر باعث می‌شود تا نتوان از اشتراک گذاری منابع مدیریتی بهره زیادی برد و بنابراین باعث
اختلاف میان سود حاصل از الگوریتم توامان و غیرتوامان می‌گردد.

\begin{figure}[!h]
\center\includegraphics[scale=0.5]{images/joint-usnet}
\caption{مقایسه راه‌حل توامان در شبکه \lr{USnet}}
\label{fig.21}
\end{figure}

نکته‌ی مهم در مساله‌ی غیرتوامان در اولین مرحله‌ی آن می‌باشد که ممکن است تعداد زیادی زنجیره را پذیرفته و به این ترتیب کار را در مرحله‌ی دوم پیچیده کرده و باعث رد شدن تعداد زیادی از زنجیره‌های انتخاب شده گردد.
بنابراین همانطور که پیشتر بیان شد نیاز است که به مساله امکان در نظر نگرفتن انتخاب‌های مرحله‌ی قبلی داده شده یا به اندازه‌ی نیاز منابع مدیریتی از پیش در نظر گرفته شوند.
